...
Base = db.Model
class BaseMixin:
    create_time = db.Column(db.DateTime, default=datetime.datetime.now(datetime.UTC))
    update_time = db.Column(db.DateTime, default=datetime.datetime.now(datetime.UTC), onupdate=datetime.datetime.now(datetime.UTC))
...
class ServiceEnv(db.Model, BaseMixin):
    id = db.Column(db.Integer, primary_key=True)
    service_env_name = db.Column(db.String(80), nullable=False) # 保留值有uat、prod等，避免新建
    ...
    attributes = db.Column(db.String(120), default="{}", nullable=False)  # Json 格式扩展，可以通过Service继承部分配置
...
class UserBindLane(db.Model, BaseMixin):
    id = db.Column(db.Integer, primary_key=True)
